...
/*
 * function window.onhelp(){return false} function KeyDown(){ if
 * ((window.event.altKey)&&((window.event.keyCode==37)||(window.event.keyCode==39))){
 * event.returnValue=false; } if((event.keyCode==116)||(event.ctrlKey &&
 * event.keyCode==82)){ event.keyCode=0; event.returnValue=false; } if
 * ((event.ctrlKey)&&(event.keyCode==78)){ event.returnValue=false; } if
 * ((event.shiftKey)&&(event.keyCode==121)){ event.returnValue=false; } if
 * (window.event.srcElement.tagName == "A" && window.event.shiftKey){
 * window.event.returnValue = false; } if
 * ((window.event.altKey)&&(window.event.keyCode==115)){
 * window.showModelessDialog("about:blank","","dialogWidth:1px;dialogheight:1px");
 * return false; } } document.onkeydown=KeyDown;
 * 
...
/*
 * @date:2004-5-9 10:01 @author:shenqin
 */
 ...
 * 
 * 
 * function checkTimeField(formName,hourname,minutename,fieldDesc){ if
 * (checkNullField(formName,hourname,fieldDesc) == false ) { return false; } if
 * (checkNullField(formName,minutename,fieldDesc) == false ) { return false; }
 * if (checkIntField(formName,hourname,fieldDesc) == false ) { return false; }
 * if (checkIntField(formName,minutename,fieldDesc) == false ) { return false; }
 * if ((eval(formName+'.'+hourname+'.value<0')) ||
 * (eval(formName+'.'+hourname+'.value>23')) ) { alert('请输入有效的时间！');
 * eval(formName+"."+hourname+".focus()"); return false; } if
 * ((eval(formName+'.'+minutename+'.value<0')) ||
 * (eval(formName+'.'+minutename+'.value>59')) ) { alert('请输入有效的时间！');
 * eval(formName+"."+minutename+".focus()"); return false; } return true; }
 */
 
/**
 * 2003-7-29 22:14 新增时间校验函数
 */
...
/*
 * function checkTimeField(formName,fieldName,fieldDesc){ var
 * timeStr=trim(eval("document."+formName+"."+fieldName+".value"));
 * if(checkInvalid(timeStr)) {
 * if(timeStr.split(":")[0]/1>23||timeStr.split(":")[0]/1<0||isNaN(timeStr.split(":")[0])){
 * alert(fieldDesc+" 必须是有效的时间！"); eval(formName+"."+fieldName+".focus()");
 * return false; } if(timeStr.split(":")[1]/1>59||timeStr.split(":")[1]/1<0||isNaN(timeStr.split(":")[1])) {
 * alert(fieldDesc+" 必须是有效的时间！"); eval(formName+"."+fieldName+".focus()");
...
/**
 * fuctonName: comptime 2003-6-23 05:16下午 新增时间比较函数
 */
...
/*
 * fieldName: hiddenFieldName fieldTextName: textFieldName
 */
...
/*
 * @date:2004-5-15 17:32 @author:shenqin @function: 获取对象属性
 */
...
/*
 * date:2004-2-28 16:17 author:zxcshenqin@163.com
 */
...
/*
 * @date:2004-6-2 10:28 @author:shenqin @function: 重载函数，增加参数
 */
   ...
   var W= new Array(7,9,10,5,8,4,2,1,6,3,7,9,10,5,8,4,2,1); 
	...
	var newss=ss;
 	if(ss.length==15)
  			...
  			return false;
      		...
      		else if(ss.length==18)
       		{
       		   ...
       		   { 
  			      ...
  			      {
  			        var conf = window.confirm("请输入您的身份证号码中的校验位号码错误，点击【确认】修改，点击【取消】继续操作");
		  			if(conf==true){
						eval("document."+formName+"."+fieldName+".value=''");
						eval("document."+formName+"."+fieldName+".focus()");
						return false;
  	...
  	{
  		var conf = window.confirm("您所输入的身份证号码错误，点击【确认】修改，点击【取消】继续操作");
		if(conf==true){
			eval("document."+formName+"."+fieldName+".value=''");
			eval("document."+formName+"."+fieldName+".focus()");
			return false;
...
/*
 * formName fieldName textField fieldTextName HiddenField fieldDesc textDesc
...
/*
 * @date:2004-5-12 8:56 @author:shenqin @vision:v3.3.2 @function: 返回链接所需要的url字符串
 */
...
/*
 * date:2004-2-16 10:37 fun: test formName,fieldName is checked
 */
  	...
  	if (ObjID != -1 && DesObjID != -1){  
        ...
        { // if (eval("document."+formName).elements[DesObjID].length == 5)
          ...
          // else
           ...
           // if ((SelNum +
			// eval("document."+formName).elements[DesObjID].length) > 5)
          // else
              {  // add
...
/**
 * date:2003-9-20 9:49 处理鼠标over的事件
 */
...
/*
 * date:2004-4-29 8:56 @formName @fieldName @fieldTextName return true or false;
 * function:清除隐藏域
 */
