...
public class MessageServiceImpl extends BaseServiceAbstractSupport {
	...
	public void saveOrUpdate(Object myaction) throws Exception {
		...
		String str_maxfilesize = CommonQuery.getSysProperty(SysConstants.MESSAGE_FILE_SIZE);
		long maxFileSize;
		if (StringUtils.isBlank(str_maxfilesize)) {
			maxFileSize = SysConstants.FILE_SIZE_1M;// 用系统默认，已经按字节计算
		} else {
		...
		if (!SysConstants.SYSCODE_XXLY_QFXSXX.equals(object.getXxly())) {
			for (int i = 0; i < xxjsr.length; i++) {
				TSysMessageGxqf tsysMessageGxqf = new TSysMessageGxqf();
				tsysMessageGxqf.setXxzt(SysConstants.MESSAGE_STATE_UNREAD);
