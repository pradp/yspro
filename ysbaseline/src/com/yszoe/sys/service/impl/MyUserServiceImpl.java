...
import com.yszoe.framework.identity.service.impl.IdUserServiceImpl;
...
import com.yszoe.sys.action.MyUserAction;
...
public class MyUserServiceImpl extends IdUserServiceImpl {
	...
	public boolean remove(Object myaction) throws Exception {
		...
		boolean f = false;
		if (SysConstants.MESSAGE_RETURN_SUCCESS_CODE.equals(mes)) {
			f = super.remove(myaction);
		} else if (SysConstants.MESSAGE_RETURN_FAILURE_CODE.equals(mes)) {
			action.addActionMessage("用户已在使用中，目前不能删除！");
			f = false;
		}
		if (f) {
			BusinessLogUtil.log(action.getLoginUser().getUserLoginId(),
					SysConstants.CZDX_T_SYS_USER, SysConstants.LOG_ACTION_DEL);
		}
		return f;
	}
		...
		if (tsysUser == null) {
			...
			if (tsysUser.getUserid() == null || tsysUser.getUserid().equals("")) {
				...
				if ("7".equals(action.getDepart().getDeparttype())
					...
					// 区县用户可以添加多个同级账户
					String count = PropConfigUtil.get(SysConstants.QXTJYHZDS);
					int a = 0;
					if (count != null) {
						try {
					...
					// 区县以外用户默认添加同一部门下的账户
					String count = PropConfigUtil.get(SysConstants.TJZHZDS);
					int a = 0;
					if (count != null) {
						try {
							a = Integer.parseInt(count);
				...
				BusinessLogUtil.log(action.getLoginUser().getUserLoginId(),
						SysConstants.CZDX_T_SYS_USER, SysConstants.LOG_ACTION_SAVE);
				...
				BusinessLogUtil.log(action.getLoginUser().getUserLoginId(),
						SysConstants.CZDX_T_SYS_USER, SysConstants.LOG_ACTION_UPDATE);
			}
		}
