...
public class OpenTimeCtrlUtil {
	...
	public static boolean checkButtonPermission(TSysButtonRule tsysButtonRule){
		...
		if(StringUtils.isNotBlank(lx)){
			...
			if(SysConstants.BUTTON_RULE_EACH_DAY == cycleType){//每天。查询当前时间是否在规则定义的时间段范围之内。
			...
			}else if(SysConstants.BUTTON_RULE_EACH_MONTH == cycleType){//每月。查询当前日是否在规则定义的日范围之内。
			...
			}else if(SysConstants.BUTTON_RULE_EACH_YEAR == cycleType){//每年。查询当前月日是否在规则定义的月日范围之内。
