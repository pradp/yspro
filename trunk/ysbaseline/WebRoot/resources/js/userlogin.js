...
function checkUserInSession(userLoginId){
    ...
    var ajaxLoginError = function(XMLHttpRequest, textStatus, errorThrown){
        if (textStatus == "timeout") {
            $.unblockUI();
            var say = "操作超时！<br/><br/>请检查网络连接是否正常，或者联系管理员帮您解决这个问题！";
    ...
    var handleResponse = function(data){
        if (data.indexOf("true") != -1) {
            $.unblockUI();
            var say = "另一用户已使用此账号登录！<br/><br/>如果要继续登录（另一用户会退出，可能会造成他操作的数据不完整！），请点击确定。<br/><br/>返回到登录页面并输入不同的用户标识，请点击取消。";
...
function executeLogin(){
    ...
    var handleResponse = function(data){
        ...
        if (data.indexOf("开启菜单栏") != -1) {
            //登录成功
            window.location.href = basePath + "/identity/index.c";
        }else{
