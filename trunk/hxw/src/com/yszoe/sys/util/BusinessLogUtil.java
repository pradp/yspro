...
public class BusinessLogUtil implements Runnable {
	...
	private static final String sql = SQLConfigUtil.getSql("sql.bizlog.insert1");
	private static final String sql2 = SQLConfigUtil.getSql("sql.bizlog.insert2");
	// TODO sql增加一个描述字段
	...
	public BusinessLogUtil(String opUserLoginid, Object opTableObject, String clientIp, int opType) {
		super();
	...
	public BusinessLogUtil(String opUserLoginid, Object opTableObject, int opType, String memo) {
		super();
		this.opUserLoginid = opUserLoginid;
		this.opTableObject = opTableObject;
		this.opType = opType;
	 ...
	 */
	public static final void log(String opUserLoginid, Object opTableObject, int opType, String clientIp) {
		BusinessLogUtil bizLogUtil = new BusinessLogUtil(opUserLoginid, opTableObject, clientIp, opType);
		Thread t = new Thread(bizLogUtil, "iBizLogThread");
		t.setPriority(Thread.MIN_PRIORITY);
		t.start();
	}

	/**
	 * 记录好友足迹/动作 此方法是静默执行的，不会抛出任何异常（包括运行时异常）。
	 * 
	 * @param opUserLoginid 操作人的登录ID
	 * @param memo 备注
	 * @param opType 操作类型（保存1、更新2、删除3）
	 */
	public static final void action(String opUserLoginid, Object opTableObject, String czbid, int opType, String type,
			String czms, String cznrzy) {
	...
	public void run() {
		String wid = SeqFactory.getNewSequenceAlone();
		try {
			DBUtil.executeSQL(sql2, wid, opTableObject, opUserLoginid, opType, clientIp);
		} catch (RuntimeException e) {
			log.error("为[" + opUserLoginid + "， " + String.valueOf(opTableObject) + "， " + opType + "]记录日志的时候发生运行时异常：");
			log.error(e);
		} catch (Exception e) {
