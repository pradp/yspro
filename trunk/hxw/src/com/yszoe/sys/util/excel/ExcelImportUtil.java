...
public class ExcelImportUtil {
	...
	private List<String[]> getFastExcelContext(String filePath, int sheetIndex) throws Exception {
		...
		try{
			wookBook = Workbook.getWorkbook(new File(filePath));// 获取Excel文件
			Sheet sheet = wookBook.getSheet(sheetIndex);// 获取指定工作区
			int colNumber=0;
			...
			for(int i=startRow;i<sheet.getRows();i++){
				...
				if(isFirst){
					colNumber=cell.length;
					isFirst=false;
				}else{
					if(colNumber<cell.length){//当前行的列数和标题行列数不一致，说明模板被变动过
						throw new Exception("模板错误：当前行的列数和标题行列数不一致！");
					}
				}
				...
				for(String s:sub){//这里检查一下，一行中只要有一列是有值的，就认为是有效行。
			...
			if(wookBook!=null)wookBook.close();
		}
